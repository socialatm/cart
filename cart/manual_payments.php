...
function cart_checkout_manual (&$hookdata) {
	...
	$order["finishedurl"]= z_root() . '/cart/' . $nick;
   	$order["links"]["checkoutlink"] = z_root() . '/cart/' . $nick . '/checkout/start?cart='.$order["order_hash"];
        $template = Theme::get_template('basic_checkout_manual_confirm.tpl','addon/cart/');
...
function cart_manualpayments_unload () {
    ...
    Code\Extend\Hook::unregister_by_file('addon/cart/manual_payments.php');

    }

function cart_manualpayments_load () {

    Code\Extend\Hook::register('cart_paymentopts','addon/cart/manual_payments.php','cart_paymentopts_register_manual');
    Code\Extend\Hook::register('cart_checkout_manual','addon/cart/manual_payments.php','cart_checkout_manual');
    Code\Extend\Hook::register('cart_post_manual_checkout_confirm','addon/cart/manual_payments.php','cart_post_manual_checkout_confirm');
